...
export default class Step {
    ...
    constructor(shape: ShapeInstance, cfg: Config) {
        this.shape = shape
        this.cfg = cfg
    ...
    compute(state: State) {
        ...
        this.color = color
        let currentDifference = util.distanceToDifference(
            state.distance,
            pixels
        )
    ...
    mutate() {
        let newShape = this.shape.mutate()
        let mutated = new Step(newShape, this.cfg)
