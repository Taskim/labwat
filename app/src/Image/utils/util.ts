import { ImageDataSets, BoundingBox } from '../types'
...
export function clampColor(x: number): number {
    return clamp(x, 0, 255)
}

export function distanceToDifference(distance: number, pixels: number): number {
    return Math.pow(distance * 255, 2) * (3 * pixels)
}

export function differenceToDistance(
    difference: number,
    pixels: number
): number {
...
export function difference(data: ImageData, dataOther: ImageData): number {
    let sum = 0,
        diff
...
function computeColor(
    ...
    for (sy = 0; sy < sh; sy++) {
        ...
        for (sx = 0; sx < sw; sx++) {
            ...
            color[0] +=
                (targetData[fi] - currentData[fi]) / alpha + currentData[fi]
            color[1] +=
...
function computeDifferenceChange(
    offset: BoundingBox,
    imageData: ImageDataSets,
    color: any
) {
...
export function computeColorAndDifferenceChange(
    offset: BoundingBox,
    imageData: ImageDataSets,
    alpha: number
) {
